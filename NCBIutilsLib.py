...
# converted to python3 - jak, 4/10/2020
#       this included adding optional rettype param to the getResults()
#       methods.
...
ESUMMARY_BASE = EUTILS_BASE + 'esummary.fcgi?&api_key=' + EUTILS_API_KEY
        ...
        # not used yet. Should implement functions for this. Correct URL?
ID_CONVERTER_URL = ESEARCH_BASE + '&db=pubmed&term=%s[lid]'

USEHISTORY = "&usehistory=y"	# eutils param for history

DEFAULT_ENCODING = 'utf-8'      # to use when creating URLs
# -------------------------
...
def doPost(db,		# eutils db name ('pubmed', 'pmc', ...)
    ...
    otherParams = b'api_key=%b&db=%b&id=' %(toBytes(EUTILS_API_KEY),toBytes(db))
...
if __name__ == "__main__":      # test code
    ...
    if True:
        ...
        count, webenv = doSearch('pubmed', query, URLReader=URLReader,
            debug=False)
        ...
        count, output, webenv = getSearchResults('pubmed',query, op='fetch',
            rettype='medline', retmode='text', URLReader=URLReader, debug=False)
        print("output: \n%s" % output[:500])
        print()
    ...
    if True:    # post tests
        ids = [28440906, 28256074, ]
        ids = ['28440906', '28256074', ]
        ids = [b'28440906', b'28256074', ]
        print('-' * 30 + " Post pubmed")
        webenv = doPost('pubmed', ids, URLReader=URLReader, debug=True)
        print("webenv: '%s'" % webenv)
        print()
